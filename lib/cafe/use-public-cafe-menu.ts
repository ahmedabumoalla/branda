...
function normalizePayload(
  json: Partial<PublicMenuPayload> | null | undefined,
): PublicMenuPayload {
  return {
    ...
    loyaltyRewards: Array.isArray(json?.loyaltyRewards)
      ? json.loyaltyRewards
      : [],
    experienceCampaigns: Array.isArray(json?.experienceCampaigns)
      ? json.experienceCampaigns
      : [],
  };
}
...
function resourceUrl(slug: string, options: Options) {
  ...
  if (options.resource === "offers")
    return `${base}/offers?limit=${options.limit ?? 12}`;
...
async function loadResource(slug: string, options: Options) {
  const response = await fetch(resourceUrl(slug, options), {
    cache: "no-store",
  });
  if (!response.ok) {
    throw new Error(
      response.status === 404
        ? "الفرع أو المورد غير موجود"
...
export function prefetchPublicCafeResource(
  slug: string,
  options: Options = {},
) {
  if (!isSupabaseConfigured()) return Promise.resolve(emptyPayload);
  return cachedRequest(resourceKey(slug, options), TTL_MS, () =>
    loadResource(slug, options),
  );
}
...
export function usePublicCafeMenu(slug: string, options: Options = {}) {
  ...
  const key = resourceKey(slug, resolvedOptions);
  const initial = enabled ? readMemoryCache<PublicMenuPayload>(key) : null;
  const [data, setData] = useState<PublicMenuPayload>(
  ...
  useEffect(() => {
    ...
    const cached = readMemoryCache<PublicMenuPayload>(
      resourceKey(slug, resolvedOptions),
    );
    if (cached) {
      setData(cached);
      setLoading(false);
  ...
  }, [slug, resource, productId, limit, enabled]);
