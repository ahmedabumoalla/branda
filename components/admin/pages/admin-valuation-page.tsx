...
export function AdminValuationPage({ data, configError }: { data: BarndaksaFinanceDashboard | null; configError?: string }) {
  return (
    <AdminPageShell title="تفصيل آلية تقييم بارنداكسا" subtitle="صفحة مستقلة توضح المعايير العالمية المستخدمة في قراءة القيمة السوقية الحالية للمنصة." action={<BarndaksaLogo variant="dark" width={140} height={56} />}>
      ...
      <BentoGrid className="mb-6">
        <BentoCard variant="gold"><p className="text-sm font-bold text-[#241610]">القيمة السوقية الحالية للبراند اللي هو بارنداكسا</p><h2 className="mt-2 text-4xl font-black text-[#241610]">{data ? money(data.currentMarketValue) : "—"}</h2></BentoCard>
      ...
      <BentoCard variant="dark" className="mt-6">
        ...
        <div className="mt-4 space-y-3 text-sm font-bold leading-8 text-[#CBB29C]">
          ...
          <p>يتم خصم مخاطر المرحلة المبكرة لأن بارنداكسا ما زالت في مرحلة توسع، لذلك التقييم المعروض داخل المالية يعتبر قراءة إدارية داخلية وليس تقييمًا استثماريًا نهائيًا.</p>
        </div>
      </BentoCard>
    </AdminPageShell>
  );
