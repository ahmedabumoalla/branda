...
declare global {
  interface Window {
    ...
    __barndaksaPaypalLoader?: Promise<void>;
  }
}
...
function loadPaypalSdk(clientId: string) {
  ...
  if (window.__barndaksaPaypalLoader) return window.__barndaksaPaypalLoader;
  ...
  script.async = true;
  script.dataset.barndaksaPaypal = "true";

  window.__barndaksaPaypalLoader = new Promise<void>((resolve, reject) => {
    script.onload = () => resolve();
    script.onerror = () => reject(new Error("تعذر تحميل بوابة الدفع"));
    document.body.appendChild(script);
  });

  return window.__barndaksaPaypalLoader;
}

export function BarndaksaCardPaymentButton({ subscriptionId, disabled, onPaid, onMessage }: Props) {
    ...
    void loadPaypalSdk(clientId)
      .then(async () => {
        ...
        buttons = window.paypal.Buttons({
          ...
          onError: (paypalError) => {
            console.error("[BarndaksaCardPaymentButton]", paypalError);
            setProcessing(false);
      ...
      .catch((loadError) => {
        console.error("[BarndaksaCardPaymentButton:load]", loadError);
