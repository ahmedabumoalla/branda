# Branda — Staging Deployment Guide

> **تحذير:** هذا الدليل للبيئة **المحلية (Local)** ومشروع **Supabase Staging** فقط.  
> **لا تشغّل** `supabase db push` أو `db reset` على Production قبل مراجعة [`BRANDA_STAGING_SECURITY_VALIDATION_REPORT.md`](./BRANDA_STAGING_SECURITY_VALIDATION_REPORT.md).
...
1. Dashboard → New project → **`branda-staging-security`** (أو اسم واضح)
...
- [`BRANDA_PRODUCTION_SECURITY_TEST_CHECKLIST.md`](./BRANDA_PRODUCTION_SECURITY_TEST_CHECKLIST.md) §8 + §11
- Security Advisor + Performance Advisor
- سجّل النتائج في [`BRANDA_STAGING_SECURITY_VALIDATION_REPORT.md`](./BRANDA_STAGING_SECURITY_VALIDATION_REPORT.md)
...
1. [Supabase Dashboard](https://supabase.com/dashboard) → New project → **Staging** (مثلاً `branda-staging`).
...
5. وثّق النتائج في [`BRANDA_STAGING_SECURITY_VALIDATION_REPORT.md`](./BRANDA_STAGING_SECURITY_VALIDATION_REPORT.md)
...
- [ ] Staging: checklist §8 + §11 في [`BRANDA_PRODUCTION_SECURITY_TEST_CHECKLIST.md`](./BRANDA_PRODUCTION_SECURITY_TEST_CHECKLIST.md)
- [ ] مراجعة [`BRANDA_SECURITY_DEFINER_REVIEW.md`](./BRANDA_SECURITY_DEFINER_REVIEW.md)
