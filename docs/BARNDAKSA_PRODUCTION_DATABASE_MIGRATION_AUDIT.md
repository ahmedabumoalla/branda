# Barndaksa — Production Database Migration Audit
...
| **IndexedDB** | `barndaksa-local-assets` → store `assets` (`lib/cafe/local-asset-store.ts`) |
| **lib/mock/** | 16 ملفًا — مصدر بيانات + أنواع |
| **مفاتيح localStorage** | 30+ مفتاحًا، معظمها hardcoded لـ `qatrah` |
| **Auth** | `mockAuthUsers` + `barndaksa_auth_session` في localStorage |
| **عميل الكوفي** | `barndaksa_customer_session_{slug}` في localStorage |
| **⚠️ تصادم** | `barndaksa_qatrah_orders` يُستخدم لـ `CafeOrder[]` و`CustomerOrder[]` |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| ثيم نشط | `lib/mock/cafe-theme.ts` | localStorage | `barndaksa_qatrah_theme` | `cafe_settings.theme_id` + seed `cafe_themes` | seed ثيمات؛ PATCH theme |
| هوية مخصصة | `lib/mock/custom-identity-theme.ts` | localStorage | `barndaksa_qatrah_custom_identity_theme` | `cafe_custom_identity` | palette jsonb؛ صور → Storage |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| منتجات | `lib/mock/menu.ts` | LS + mock | `barndaksa_qatrah_menu` | `menu_products` | CRUD + `category_id` FK |
| فئات | `lib/mock/menu-categories.ts` | LS + defaults | `barndaksa_qatrah_menu_categories` | `menu_categories` | migrate قبل products |
| عروض | `lib/mock/offers.ts` | LS + mock | `barndaksa_qatrah_offers` | `offers` | banner → `offer-banners` |
| صور منتج | `product-modal.tsx` | IndexedDB | `barndaksa-qatrah-product-{id}-image` | `menu-products/{cafe_id}/` | signed upload |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| طلبات | `lib/mock/orders.ts` | LS + mock | `barndaksa_qatrah_orders` | `orders`, `order_items` | مصدر واحد؛ soft delete |
...
| لوحة | `orders-page.tsx` | localStorage | `barndaksa_qatrah_orders` | `orders` | fetch + PATCH status |

### الحجوزات

| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| حجوزات | `lib/mock/reservations.ts` | LS + mock | `barndaksa_qatrah_reservations` | `reservations`, `reservation_responses` | status server-enforced |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| إعدادات | `lib/mock/loyalty.ts` | LS + mock | `barndaksa_qatrah_loyalty_settings` | `loyalty_rules` (jsonb) | owner CRUD |
| مكافآت | `lib/mock/loyalty.ts` | LS + mock | `barndaksa_qatrah_loyalty_rewards` | `loyalty_rewards` | derive from rules |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| تسويق | `lib/mock/marketing.ts` | LS + mock | `barndaksa_qatrah_marketing` | `marketing_campaigns` | CRUD + images |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| إشعارات | `lib/mock/notifications.ts` | localStorage | `barndaksa_qatrah_notifications` | `notifications` | **لا insert من client** |
...
| خطط | `lib/platform/admin-data.ts` | LS + mock | `barndaksa_platform_plans` | `platform_plans` | admin CRUD |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| جلسة عميل | `lib/customer/session.ts` | localStorage | `barndaksa_customer_session_{slug}` | Supabase Auth + `customer_profiles` | OTP/email auth |
| ملفات | `lib/mock/customer-activity.ts` | LS | `barndaksa_customers_qatrah` | `customer_profiles` | scoped by cafe |
| مراجعات | `lib/mock/reviews.ts` | LS + mock | `barndaksa_qatrah_reviews` | `reviews` | moderation API |
...
| المجال | الملف الحالي | نوع التخزين الحالي | المفتاح أو المصدر الحالي | الجدول أو Storage Bucket البديل | الإجراء المطلوب |
|--------|-------------|-------------------|-------------------------|------------------------------|-----------------|
| Auth | `lib/platform/auth.ts` | LS + hardcoded passwords | `barndaksa_auth_session` | Supabase Auth + `profiles` | إزالة mock passwords |
| مقاهٍ | `lib/platform/admin-data.ts` | LS + mock | `barndaksa_platform_cafes` | `cafes` | admin API + RLS |
| عمليات | admin-operations-page | LS | `barndaksa_platform_operations` | `audit_logs`, `platform_operations` | immutable audit |
...
|-------|------------------|--------|
| شعار كوفي | `barndaksa-qatrah-cafe-logo` | `cafe-logos` |
...
1. تطبيق `001_barndaksa_production_schema.sql` في Supabase Dashboard
...
**مرجع:** `docs/BARNDAKSA_DATABASE_BLUEPRINT.md` — أقسام 8, 12, 15, 16
