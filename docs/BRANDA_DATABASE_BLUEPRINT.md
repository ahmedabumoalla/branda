# BRANDA Database Blueprint

## 1) Executive Summary

هذا المستند هو مخطط قاعدة البيانات والأمن والتخزين المستقبلي لمنصة `branda-platform` بناءً على مراجعة الكود الحالي (app/components/lib/api/docs).  
...
- `branda_qatrah_settings` -> `cafe_settings`
- `branda_qatrah_theme` -> `cafe_settings.theme_id` + `cafe_theme_history`
- `branda_qatrah_menu` -> `menu_products`
- `branda_qatrah_offers` -> `offers`
- `branda_qatrah_orders` -> `orders/order_items`
- `branda_qatrah_invoices` -> `invoices`
- `branda_qatrah_customer_transactions` -> `customer_transactions`
- `branda_qatrah_reservations` -> `reservations`
- `branda_qatrah_reviews` -> `reviews/review_replies`
...
- `branda_qatrah_pending_subscription` -> `payment_events`/pending checkout table
- `branda_platform_*` -> platform admin tables
- domain keys (`domain_searches/purchases/active`) -> `domain_orders/domain_events/cafe_domains`
- `branda_customer_session_${slug}` -> secure server session/JWT + `customer_sessions`
...
|-----|------|------------|---------|-----------|
| `branda_qatrah_menu_categories` | `MenuCategoryRecord[]` | `menu-page.tsx` | menu, cafe public, custom theme | `product_categories` |
| `branda_qatrah_custom_identity_theme` | `CustomIdentityTheme` | `custom-identity-builder.tsx`, `theme-page.tsx` | `brand-identity-custom-theme.tsx` | `cafe_custom_identity` |
| `branda_qatrah_experience_campaigns` | `ExperienceCampaign[]` | `marketing-page.tsx` | marketing, loyalty flows | `experience_campaigns` |
| `branda_qatrah_experience_submissions` | `ExperienceSubmission[]` | marketing + approval flows | marketing, notifications | `experience_submissions` |
| `branda_qatrah_notifications` | `AppNotification[]` | order/reservation/experience flows | dashboard (future bell), account | `notifications` |

### Extended Keys (schema changes)

| Key | New fields / behavior | DB columns |
|-----|----------------------|------------|
| `branda_qatrah_menu` | `categoryId`, `availableForPickup`, `pickupLeadTimeMinutes` on `MenuProduct` | `menu_products.category_id`, `available_for_pickup`, `pickup_lead_minutes` |
| `branda_qatrah_orders` | Pickup-only: `type=استلام`, `pickupAt`, `status`, `paymentStatus`, `rejectionReason`, `cafeResponseAt` | `orders.fulfillment_type`, `pickup_at`, `status`, `payment_status`, `rejection_reason`, `responded_at` |
| `branda_qatrah_theme` | New theme id `brand-identity-custom` (11th theme) | `cafe_themes` seed + `cafe_settings.theme_id` |
...
|--------------|-------|-------|
| `branda_qatrah_menu_categories` | `product_categories` | migrate before products FK |
| `branda_qatrah_custom_identity_theme` | `cafe_custom_identity` | palette jsonb; images → Storage |
| `branda_qatrah_experience_campaigns` | `experience_campaigns` | |
| `branda_qatrah_experience_submissions` | `experience_submissions` | approval workflow server-side |
| `branda_qatrah_notifications` | `notifications` | replace client-only writes with triggers |
| `branda_qatrah_orders` (pickup fields) | `orders` + optional `order_fulfillment` | status transitions server-enforced |
...
- Client hook cached default theme from SSR via `useMemo([previewTheme])` without re-reading `branda_qatrah_theme` after hydration.
...
|-----|----------|---------|---------|
| `branda_qatrah_theme` | `CAFE_THEME_KEY` | `lib/cafe/theme-storage-sync.ts` (`adoptCafeTheme`), `theme-page.tsx`, `custom-identity-builder.tsx` | `use-resolved-cafe-theme.ts`, `use-cafe-theme-page.ts`, `cafe-theme-renderer.tsx` |
| `branda_qatrah_custom_identity_theme` | `CUSTOM_IDENTITY_THEME_KEY` | `persistCustomIdentityTheme` in `theme-storage-sync.ts` | `brand-identity-custom-theme.tsx`, `themed-cafe-shell.tsx`, `custom-identity-featured.ts` |
| `branda_qatrah_menu_categories` | `MENU_CATEGORIES_KEY` | `menu-page.tsx` via `saveMenuCategories` + `notifyMenuCategoriesUpdated` | `menu-category-utils.ts`, `ThemeCategoryStrip`, `product-collection-page.tsx` |

### Cross-tab / same-tab sync

Custom events (same tab): `branda:theme-updated`, `branda:custom-identity-updated`, `branda:menu-categories-updated`.  
...
- **`cafe_custom_identity`** ← `branda_qatrah_custom_identity_theme` (palette jsonb; `logo_url`, `background_url` in Storage — **not** base64 in DB).
- **`product_categories`** ← `branda_qatrah_menu_categories`.
- **`cafe_themes.active_theme_id`** ← `branda_qatrah_theme` value e.g. `brand-identity-custom`.
...
`QuotaExceededError` when saving `branda_qatrah_custom_identity_theme` because logo/background were stored as Base64 Data URLs in `localStorage` (same risk for `branda_qatrah_settings.logoDataUrl`).
...
| **IndexedDB** `branda-local-assets` → store `assets` | Image Blobs (client-only mock) |

Fixed asset IDs (replace in place, no accumulation):

- `branda-qatrah-custom-theme-logo`
- `branda-qatrah-custom-theme-background`
- `branda-qatrah-cafe-logo`
...
|-------------|--------------------------------|
| `branda_qatrah_custom_identity_theme` | `logoAssetId`, `backgroundAssetId` |
| `branda_qatrah_settings` | `logoAssetId` |
| `branda_qatrah_menu` | `MenuProduct.imageAssetId` |
| `branda_qatrah_menu_categories` | `MenuCategoryRecord.imageAssetId` |
| `branda_qatrah_offers` | `CafeOffer.bannerAssetId` + http `bannerImageUrl` |
| `branda_customer_session_{slug}` | `avatarAssetId` |

External **http(s) URLs** remain valid without asset IDs (mock CDN / Unsplash defaults).

### IndexedDB store

- DB: `branda-local-assets`, store: `assets`, keyPath: `id`
