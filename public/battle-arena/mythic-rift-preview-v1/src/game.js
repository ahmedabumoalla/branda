...
function updateUI() {
  ...
  const next = game.running ? CARDS[game.playerDeck[game.playerCursor % game.playerDeck.length]] : CARDS[0]; ui.next.src = next.image;
...
document.querySelector('#battleButton').addEventListener('click',openBattle);
document.querySelector('#collectionBattle')?.addEventListener('click',openBattle);
...
document.querySelector('#restartBattle').addEventListener('click',()=>{newGame();showScreen('battle')});
...
await loadMap();renderMetaScreens();newGame();showScreen('battle');cancelAnimationFrame(animationId);animationId=requestAnimationFrame(frame);
...
if(query.has('demo'))showScreen('battle');
