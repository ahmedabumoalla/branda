-- Fix missing platform_plans.is_default column used by admin_save_platform_plan RPC.
-- Safe to run multiple times.

alter table public.platform_plans
  add column if not exists is_default boolean not null default false;

-- Keep the column synchronized with platform_settings.default_plan_id if available.
do $$
declare
  v_default_plan_id text;
begin
  select default_plan_id into v_default_plan_id
  ...
  if v_default_plan_id is not null then
    ...
    set is_default = (id = v_default_plan_id);
  end if;
exception
  when undefined_table or undefined_column then
    -- Older databases may not have platform_settings/default_plan_id yet.
    null;
end $$;
...
-- Recreate RPC defensively so saving plans works even on databases that missed older migrations.
