...
CREATE TABLE IF NOT EXISTS public.branda_owners (
...
CREATE TABLE IF NOT EXISTS public.subscription_profit_distributions (
  ...
  owner_id uuid REFERENCES public.branda_owners(id),
...
INSERT INTO public.branda_owners (name, ownership_percent)
VALUES
  ('احمد ابومعلا', 30),
  ('عبدالله ابومعلا', 30),
  ('ماهر الشريف', 10),
...
DECLARE
  ...
  v_owner public.branda_owners%ROWTYPE;
BEGIN
  ...
  FOR v_owner IN SELECT * FROM public.branda_owners WHERE active = true ORDER BY name LOOP
    INSERT INTO public.subscription_profit_distributions (
...
ALTER TABLE public.branda_owners ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.subscription_profit_distributions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS branda_owners_admin_read ON public.branda_owners;
CREATE POLICY branda_owners_admin_read ON public.branda_owners
...
GRANT SELECT ON public.branda_owners TO authenticated;
GRANT SELECT ON public.subscription_profit_distributions TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.branda_owners TO service_role;
