...
CREATE OR REPLACE FUNCTION public.upsert_reservation_service_v3(
  p_cafe_id uuid,
  p_service_id uuid,
  p_name text,
  p_description text,
...
BEGIN
  ...
  IF NOT (public.has_cafe_permission(p_cafe_id, 'reservations') OR public.is_platform_admin()) THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;
  ...
  IF p_service_id IS NOT NULL THEN
    ...
    IF v_existing_cafe_id IS NOT NULL AND v_existing_cafe_id <> p_cafe_id THEN
      RAISE EXCEPTION 'Forbidden reservation service';
    END IF;
  END IF;

  IF p_service_id IS NOT NULL AND v_existing_cafe_id = p_cafe_id THEN
  ...
  ELSE
    INSERT INTO public.reservation_services (
      ...
      COALESCE(p_service_id, gen_random_uuid()), p_cafe_id, btrim(p_name), NULLIF(btrim(COALESCE(p_description, '')), ''), p_price,
    ...
    SET name = EXCLUDED.name,
        ...
        updated_at = now()
    WHERE public.reservation_services.cafe_id = p_cafe_id
    RETURNING id INTO v_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.upsert_reservation_service_v3(uuid, uuid, text, text, numeric, boolean, integer, text[], text[], text[], numeric, text, text, text, boolean, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.upsert_reservation_service_v3(uuid, uuid, text, text, numeric, boolean, integer, text[], text[], text[], numeric, text, text, text, boolean, integer) TO authenticated;
