-- Barndaksa Platform — Full cashier PWA loyalty system
-- Version: 014
-- TARGET: barndaksa-production
-- Run after 013_barndaksa_loyapro_wallet_loyalty_system.sql
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'total', COALESCE(o.total_amount, o.total, o.final_price, 0),
  ...
  WHERE o.cafe_id = v_session.cafe_id
    ...
    AND o.status IN ('pending', 'new', 'accepted')
  LIMIT 30;
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'customerPhone', COALESCE(r.customer_phone, ''),
  ...
  WHERE r.cafe_id = v_session.cafe_id
    ...
    AND r.status IN ('pending', 'new', 'accepted')
  LIMIT 30;
...
BEGIN
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'total', COALESCE(v_order.total_amount, v_order.total, v_order.final_price, 0),
...
REVOKE ALL ON FUNCTION public.create_cafe_cashier(uuid, text, text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.login_cafe_cashier(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.logout_cafe_cashier(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_cashier_console(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.cashier_accept_order(text, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.cashier_accept_reservation(text, uuid) FROM PUBLIC;
...
SELECT
  'full_cashier_pwa_loyalty_ready' AS status,
  EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'cafe_cashier_activity_logs'
  ) AS cashier_activity_logs_ready;
