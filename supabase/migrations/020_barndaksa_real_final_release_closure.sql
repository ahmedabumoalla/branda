-- Barndaksa real final release closure
-- Version 020
...
UPDATE public.cafes
SET public_code = upper(substr(replace(id::text, '-', ''), 1, 10))
...
WHERE loyalty_points IS DISTINCT FROM 0
   OR redeemable_with_points IS DISTINCT FROM false
   OR redemption_points IS NOT NULL;
...
CREATE POLICY home_banners_owner_all ON public.home_banners
...
DROP POLICY IF EXISTS experience_owner_read ON public.experience_proof_links;
CREATE POLICY experience_owner_read ON public.experience_proof_links
  FOR SELECT TO authenticated
  USING (public.has_cafe_access(cafe_id) OR public.is_platform_admin());

DROP POLICY IF EXISTS experience_public_insert ON public.experience_proof_links;
CREATE POLICY experience_public_insert ON public.experience_proof_links
  FOR INSERT TO anon, authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS support_admin_owner_read ON public.brand_support_tickets;
CREATE POLICY support_admin_owner_read ON public.brand_support_tickets
  FOR SELECT TO authenticated
  USING (public.is_platform_admin() OR (cafe_id IS NOT NULL AND public.has_cafe_access(cafe_id)));

DROP POLICY IF EXISTS support_public_insert ON public.brand_support_tickets;
CREATE POLICY support_public_insert ON public.brand_support_tickets
  FOR INSERT TO anon, authenticated
  WITH CHECK (true);
...
CREATE INDEX IF NOT EXISTS idx_loyalty_cards_customer_fast ON public.loyalty_cards(cafe_id, customer_profile_id, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_home_banners_public_fast ON public.home_banners(cafe_id, active, sort_order);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.home_banners TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.experience_proof_links TO authenticated;
GRANT INSERT ON public.experience_proof_links TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.brand_support_tickets TO authenticated;
GRANT INSERT ON public.brand_support_tickets TO anon;

COMMIT;

SELECT 'barndaksa_real_final_release_ready' AS status;
